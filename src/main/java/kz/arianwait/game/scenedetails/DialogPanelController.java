...
import kz.arianwait.game.buttonaction.ButtonActionRegistry;
import kz.arianwait.game.scenelist.SceneInfo;
import kz.arianwait.game.utils.UiConfigParser;
import kz.arianwait.game.utils.UiFactory;
 ...
 * дизайн-разрешения {@link kz.arianwait.game.utils.VirtualViewport}.
 */
public class DialogPanelController implements DialogPanel {
    ...
    private void handlePuzzleHotkey(javafx.scene.input.KeyEvent event) {
        if (event.getCode() == javafx.scene.input.KeyCode.ESCAPE) {
            kz.arianwait.game.panel.BaseGamePanel puzzle = SceneInfo.getActivePuzzle();
            if (puzzle != null) {
                puzzle.forceFailure();
            }
            event.consume();
    ...
    private boolean hasOverlayOpen(AppSettings appSettings) {
        return kz.arianwait.game.utils.OverlayMarker.hasOpenOverlay(appSettings.getRoot());
    }
